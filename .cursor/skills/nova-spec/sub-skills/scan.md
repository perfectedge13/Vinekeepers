...
- **Bot ids are not feature slugs:** Do **not** create a candidate feature whose slug equals a **bot id**. Bot ids are read from the bots config file: **@.cursor/project.yml** `scan.bots_config` if present (e.g. `config/bots.yaml`), else default to `config/bots.yaml` when that path exists; parse the file and collect `bots[].id`. If a logical group would produce a slug that equals one of those ids (e.g. luna), do **not** emit a separate candidate feature for it; instead assign that group's requirement_ids and asset_ids to the appropriate **capability** candidate features (e.g. bot, workflow, config). When requirement id is REQ-&lt;BOTID&gt;-* or assets are ASSET-&lt;BOTID&gt;-*, treat as bot-instance: merge into bot/workflow/config features, no standalone feature slug. If no bots config path is set or the file is missing, skip this filter.
- Output: list of candidate features with suggested_domain so the update phase can infer domains and categorize.
