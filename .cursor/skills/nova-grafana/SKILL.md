...
## Scripts reference (Docker/grafana)

| Script | Purpose |
|--------|--------|
| `apply-status-fixes.py` | Apply deterministic fixes to status.json (app query excludes datavine, row without datasource, Datavine row y=6, bump version). Run from Docker/grafana. |
| `scripts/delete-status-dashboards-from-db.sh` | **Remove duplicate dashboards:** deletes the three status dashboards from Grafana SQLite DB. Run with Grafana **stopped**. Discovers volume automatically. |
| `scripts/remove-duplicate-status-dashboards.sh` | Only moves status.json ↔ status.json.off. Does **not** remove duplicates (Grafana does not delete when file is removed). |
| `scripts/delete-duplicate-status-dashboards.py` | Lists dashboards with title "Services status (scalable)" via API. Cannot delete provisioned dashboards; run with Grafana up only if you need to list. |
| `verify-provisioning.sh` | Prints host file uid/version/query and in-container check commands. |
...
- Remove duplicate status dashboards (Grafana stopped):
  - `cd /home/perfect_edge13/Docker/grafana && sudo docker compose stop grafana && ./scripts/delete-status-dashboards-from-db.sh && sudo docker compose up -d`
