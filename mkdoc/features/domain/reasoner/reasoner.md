...
Reasoner interface for bot decisions (REQ-REASONER-001). `Reasoner.reason(ReasonerInput)` receives workflow context, current state, and the last normalized user message, then returns `ReasonerOutput` with reply text, state patches, and proposed tool calls. The engine can persist the patch, execute approved proposed tools through `ToolRunner`, and use the resulting reply when the workflow stays silent.
