...
| ASSET-OUTBOUND-DELIVERY-ROUTER | Routes outbound Discord delivery; resolves sender from ReplyTarget and lifecycle context (configuredBotId); getGatewayForChannel returns null when lifecycle bot has no gateway; getDiscordUserIdForBot(botId) returns that bot's Discord user id; default sender for non-lifecycle channels; for lifecycle channels uses configured bot's sender only, no silent fallback | src/main/java/com/vinekeepers/connectors/OutboundDeliveryRouter.java |
