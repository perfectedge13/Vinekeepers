...
| ASSET-OUTBOUND-DELIVERY-ROUTER | Routing + gateway resolution; implements ReplySender; optional FeatureRoomStateStore for sendAsRole(channelId, messageId, content, role) and sendAs(channelId, messageId, content, botId); registerSender(botId, ReplySender, OutboundGateway); getGatewayForChannel; getSelfUserIdForBot; used by DiscordSpaceOperations and DiscordAppReplySink; no silent fallback for lifecycle | src/main/java/com/vinekeepers/connectors/OutboundDeliveryRouter.java |
