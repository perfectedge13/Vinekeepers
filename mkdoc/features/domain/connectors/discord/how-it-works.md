...
3. The gateway converts Discord messages into internal `Event` payloads with `channelId`, `authorId`, `author` (username), `messageId`, `content`, and `mentions` derived from Discord **mentioned users** (snowflake id, account username, global display name)—**not** guild nicknames—so `discordMention` cannot be satisfied by nickname-shaped strings that are not real mention metadata.
4. Downstream routing reads both message text and the `mentions` payload field for `discordMention`. Text-side tokens are normalized only when they are numeric user/bot snowflakes after `<@` or `<@!`; `<@&…>` (roles) and other bracket forms do not count.
