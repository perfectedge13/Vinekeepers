...
| ASSET-POST-CHANNEL-MESSAGE-ACTION | Workflow action to post a message to a Discord channel; send target = deliveryChannelId or channelId (bind then state), or explicit **target: room \| thread** or **targetChannelId**; optional asBotId or asRole (precedence asBotId over asRole); explicit sender strict via sendAsExplicit/sendAsRoleExplicit | src/main/java/com/vinekeepers/workflow/actions/PostChannelMessageAction.java |
