...
- **Host vs container:** Use `docker compose -f compose.yaml -f compose.host-ops.yaml up -d` for the repo's containerized host-ops path. That override mounts `/var/run/docker.sock`, binds the repo to `/workspace/vinekeepers`, binds the sample `neo4j` / `wikijs` host stacks to `/opt/stacks/*`, and switches `DEPLOY_TARGETS_PATH` to `config/deploy-targets.docker.yaml` so direct compose resolves paths that actually exist inside the container. Prefer **direct** executor in production; Cursor Agent is LLM-mediated. Vinekeepers also logs startup warnings when a direct compose target's socket, working directory, or compose file is missing in a containerized runtime.
