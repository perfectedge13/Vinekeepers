...
- **Host vs container:** Use `docker compose -f compose.yaml -f compose.host-ops.yaml up -d` for the repo's containerized host-ops path. That override mounts `/var/run/docker.sock`, binds the repo to `/workspace/vinekeepers`, bind-mounts the neo4j / wikijs stack directories at the same absolute paths declared in `config/deploy-targets.yaml` (`/home/perfect_edge13/Docker/neo4j` and `/home/perfect_edge13/Docker/wikijs`), and switches `DEPLOY_TARGETS_PATH` to `config/deploy-targets.docker.yaml` so the `vinekeepers` target can still use an in-container repo path. Prefer **direct** executor in production; Cursor Agent is LLM-mediated. Vinekeepers also logs startup warnings when a direct compose target's socket, working directory, or compose file is missing in a containerized runtime.
