...
4. **deploy_resolve_project** — Single target in `config/deploy-targets.yaml`, or `DEPLOY_DEFAULT_PROJECT` / `GADGET_DEFAULT_PROJECT`, sets `deployTargetId` and `deployNeedsProjectPick`; otherwise user picks via **`deployTargets`**. Containerized host-ops runs may point `DEPLOY_TARGETS_PATH` at `config/deploy-targets.docker.yaml` so compose targets resolve against mounted in-container paths.
...
6. **Compose paths** — Same target resolution, **`deployComposeServices`** (allowlisted names + `_all` as **All services (full stack)**) for up, stop, and restart, thread, **`run_deploy_compose`** with bind **`composeOperation`** (`up` / `stop` / `restart` / `ps`; `ps` may omit service selection via `_all`). **`stop`** / **`restart`** apply only to the chosen service(s); on multi-service stacks, picking a single name (e.g. the wiki app) does not stop other services (e.g. Postgres). **`HostComposeOpsRunner`** runs **`docker compose`** by default or **`cursor agent --print ...`** when `hostOpsExecutor: cursor_agent` on the target.
...
- **Host vs container:** Use `docker compose up -d` from the repo root. `compose.yaml` mounts `/var/run/docker.sock`, binds the repo to `/workspace/vinekeepers`, bind-mounts the neo4j / wikijs stack directories at the same absolute paths declared in `config/deploy-targets.yaml` (`/home/perfect_edge13/Docker/neo4j` and `/home/perfect_edge13/Docker/wikijs`), switches `DEPLOY_TARGETS_PATH` to `config/deploy-targets.docker.yaml` for the `vinekeepers` target, and attaches the external Docker network `monitoring` for uptime bridges. Prefer **direct** executor in production; Cursor Agent is LLM-mediated. Vinekeepers also logs startup warnings when a direct compose target's socket, working directory, or compose file is missing in a containerized runtime.
