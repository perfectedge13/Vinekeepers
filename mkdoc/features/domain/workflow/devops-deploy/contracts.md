...
- **Env:** `DEPLOY_*` with `GADGET_*` aliases where noted in `.env.example` (`DEPLOY_TARGETS_PATH`, Ansible, host ops, Cursor Agent binary). The repo's `compose.host-ops.yaml` override sets `DEPLOY_TARGETS_PATH=/app/config/deploy-targets.docker.yaml`, mounts the Docker socket, and bind-mounts the host stack paths declared in `config/deploy-targets.yaml` for direct compose.
