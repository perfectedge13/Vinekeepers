...
Workflow state machine and config-driven runners (REQ-WORKFLOW-001, REQ-WORKFLOW-002). `Workflow<S>` remains the legacy state-machine contract, while `WorkflowRunner` is the engine-facing interface that loads session state, runs a configured or stub workflow, and returns `WorkflowRunResult`. `WorkflowRunnerFactory`, `ConfigurableWorkflowRunner`, `ConfigurableWorkflowState`, and session key strategies handle pause/resume lifecycle across conversational sessions. Workflows may set `workflowSchema: v2` with a `phases` map to use `GraphWorkflowRunner` (phase graph, optional rules, `legacy_action`, `linear_workflow_ref`, and `configurable_steps` capabilities: `linear_workflow_ref` runs a sibling linear workflow; `configurable_steps` runs inline `steps` with the parent v2 `llm` defaults and template policy on the same session store). Arrietty planning now has one authoritative spine: workspace preflight, silent synthesis (default single inner draft round before evaluation), one `PlanningEvaluationService` call, one `PlanningDecisionNormalizer` pass (ask-or-assume: ordinary ambiguity routes to one question or packet, not `planning_blocked`), deterministic AIRD delta application, one-question clarification transport, then packet/critique/review/approval routing through `planningNextPhase` (v2 `rs_evaluation_branch`) aligned with `planningNextAction`. User-facing copy avoids exposing internal parse/pipeline failure mechanics (`planningCycleUserVisibleFailure`, progress summaries, merge errors humanized via `PlanningUserFacingCopy`). `run_llm_planning_synthesis` maps hard failures to `PlanningFailureCategory` spreads (`SYNTHESIS_JSON_INVALID`, `SYNTHESIS_REPAIR_EXHAUSTED`, `SYNTHESIS_UPSERT_REJECTED`, etc.) and persists recovery metadata on `FeaturePlanState`; `hydrate_planning_session` spreads `planningFailureCategory` and `planningFailurePhase` when set so kickoff templates stay safe. `WorkflowRulesEngine` predicates include `reviewReady`, `approvalReady`, and `ledgerHasBlockingOpen` for deliberation-style transitions. Optional top-level `deliberation` YAML on v2 workflows is loaded into `WorkflowV2Model` and mirrored to session as `workflowDeliberationMetaJson` for tooling. Generic clarification ledger and progress-event types live under `com.vinekeepers.state.workflow` (`workflowUnresolvedItemsJson`, `workflowProgressEventsJson` with typed `kind` / `message` / `refs`). Planning spreads dual-write `reviewReady` vs `approvalReady` so “packet ready to read” stays separate from “launch approval”. `DeliberationEngine` maps canonical planning next actions to a generic `deliberationPhase`. Coordinator-facing template keys are allowlisted via `UserCopyContext` when templates use safe mode. The step DSL is documented separately in the Workflow Steps feature.
