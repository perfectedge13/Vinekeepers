...
For LLM-backed workflow steps only (`call_action` with `action: launch_cursor_run`), step config may include optional `model` to override the model for that step. Non-LLM step types reject `model` during workflow runner construction.
