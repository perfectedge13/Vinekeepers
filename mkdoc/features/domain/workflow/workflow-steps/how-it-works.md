...
7. **Planning cycle clarification and confidence:** **`PlanningCyclePipeline`** and **`PlanningPostDraftGovernor`** keep **at most one concrete user-facing question per cycle** while **material gaps** remain (unresolved questions, blocking issues, high-severity open assumptions, canonical gap rules, etc.): when a redraft path would otherwise assume through, **`ASK_ONE_QUESTION`** is chosen if structured material is still missing. **`run_plan_critique_and_readiness`** runs **`PlanReadinessCalculator`**, persists **`PlanConfidence`** (readiness, **confidenceScore**, **confidenceReasons**, known-vs-unknown tallies), and spreads readiness keys. **`PlanningApprovalGateSupport.validateApproveAllowed`** and **`evaluate_planning_approval_gate`** require stored **confidenceScore** ≥ **`PlanReadinessCalculator.APPROVAL_CONFIDENCE_THRESHOLD`** (**0.85**) when a score is set, in addition to packet, critique lifecycle, and **READY** checks.
