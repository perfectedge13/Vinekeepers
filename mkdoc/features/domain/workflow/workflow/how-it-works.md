...
4. `ConfigurableWorkflowRunner` (v1) resolves a session key, loads `ConfigurableWorkflowState`, executes steps in order, and returns `continue`, `waiting`, `completed`, or `error` state via `WorkflowRunResult`. `GraphWorkflowRunner` (v2) tracks `__v2_phaseId` and `__v2_pipelineIndex` in the same state bag, runs `legacy_action`, `linear_workflow_ref`, or `configurable_steps` per phase pipeline, applies optional rules for transitions, and completes when the phase id is `__v2_done`. Embedded `configurable_steps` segments now persist their own per-capability cursor and waiting metadata, so a v2 workflow can pause/resume multiple interaction microflows without falling back to one shared linear `stepIndex`.
