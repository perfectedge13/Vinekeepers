...
4. `ConfigurableWorkflowRunner` resolves a session key, loads `ConfigurableWorkflowState`, executes steps in order, and returns `continue`, `waiting`, `completed`, or `error` state via `WorkflowRunResult`.
