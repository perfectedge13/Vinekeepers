...
**Lifecycle room (Phase 1):** The Luna workflow uses the full sequence above. Provisioning workflows (e.g. using the Arrietty template) run: `create_channel` → branch on `CHANNEL_CREATE_FAILED` → `provision_bot_instance` → **create_lifecycle_context** → **create_thread** (optional; runs *after* create_lifecycle_context; bind `channelId`, `threadName`, optional `contextId` from state; `storeIn: deliveryChannelId`; uses the **lifecycle owner gateway** when the channel has a context; on success, updates the store via `setDeliveryTargetId(contextId, threadId)`) → `post_channel_message` → `launch_cursor_run`. The **done step** may use a short **main-room redirect** message (e.g. `"Launching now. See <#{{channelId}}>."`). Branch on `THREAD_CREATE_FAILED` when needed. Run state uses `LifecycleRunRecord` (optional `deliveryChannelId` for thread delivery); context uses `LifecycleContext` (with optional `deliveryChannelId` via `withDeliveryChannelId`) and `LifecycleContextStore` (`setDeliveryTargetId`). `CursorCloudRunMonitor` sends status and feedback to the thread when `deliveryChannelId` is set (messageId null).
