...
- **Workflow actions (call_action):** Bind has precedence over state for action inputs. Optional step **`model`** (Cursor model id) on `call_action` is merged as **`cursorModel`** when bind omits **`cursorModel`** and **`model`**. **`launch_cursor_run`** sets the Cursor API model using **`CursorLaunchModel.resolveForLaunch`**: `cursorModel`, else `model`, else `__botDefaultCursorModel` (from bot **`persona.model`**), else **`CURSOR_MODEL`** env. `create_channel` (Discord gateway `createTextChannel`; optional bind `lifecycleOwnerBotId`—after create, calls gateway `addPermissionOverride` for that bot's Discord user when supported; returns channel id or `CHANNEL_CREATE_FAILED` on failure; channel name derived from state when blank and normalized to Discord-safe before create), branch on `channelId == CHANNEL_CREATE_FAILED` to done, `provision_bot_instance`, `create_lifecycle_context`, **create_thread** (runs after create_lifecycle_context; bind `channelId`, `threadName`, optional `contextId` from state; uses **lifecycle owner gateway** when channel has a context; on success when contextId present, **LifecycleContextStore.setDeliveryTargetId**(contextId, threadId); returns thread id or `THREAD_CREATE_FAILED`; `storeIn` e.g. `deliveryChannelId`), `post_channel_message` (content interpolated from state + bind, bind overrides—e.g. `lifecycleBotName` in bind for room intro), `launch_cursor_run` (authoritative launch path for luna_cursor; returns ack with optional status e.g. "Status: launching"). Done step may use main-room redirect message (e.g. `"Launching now. See <#{{channelId}}>."`).
