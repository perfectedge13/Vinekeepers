...
- **Placement:** In `luna_cursor`, after **`get_profile_missing_fields`**, YAML may run Phase B when gaps exist: **`get_structured_discovery_gaps`** (storeSpread) → branch on **`discoveryHasOpenGaps`** → **`build_discovery_agenda`** (storeSpread) → **`prompt_for_field`** / **`capture_field`** for **`discoveryAnswerRaw`** → **`capture_and_apply_discovery_answer`** → **`classify_assumption_or_issue`** → **`recompute_plan_progress`** (storeSpread) → branch (loop with clear on **`discoveryAnswerRaw`** until no open gaps or **`discoveryBlockingIssueMode`**).
