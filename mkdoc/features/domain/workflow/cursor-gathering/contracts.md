...
- **Proposal pass (before gap loop):** In **`arrietty_room`** after **`get_profile_missing_fields`**: **`generate_planning_proposals`** (storeSpread: **`planningProposalsJson`**, **`planningConfirmQueueJson`**, **`planningHasConfirmPending`**, **`planningHasAutoApply`**) → **`apply_auto_planning_proposals`** (upserts **`AUTO_APPLY`** rows; rewrites **`planningProposalsJson`**) → while **`planningHasConfirmPending`**: **`build_proposal_confirm_prompt`** → **`prompt_for_field`** / **`capture_field`** (**`discoveryAnswerRaw`**) → **`resolve_proposal_confirmation`** → **`classify_assumption_or_issue`** → **`recompute_plan_progress`** → branch (clear **`discoveryAnswerRaw`** when looping). **`PlanningProposal`** JSON carries **`interaction`** **`AUTO_APPLY`** or **`CONFIRM`**.
- **Placement (gap loop):** **`get_structured_discovery_gaps`** (storeSpread) → branch on **`discoveryHasOpenGaps`** → **`build_discovery_agenda`** (storeSpread) → **`prompt_for_field`** / **`capture_field`** for **`discoveryAnswerRaw`** → **`capture_and_apply_discovery_answer`** → **`classify_assumption_or_issue`** → **`recompute_plan_progress`** (storeSpread) → branch (loop with clear on **`discoveryAnswerRaw`** until no open gaps or **`discoveryBlockingIssueMode`**).
...
- **Planning models:** Typed **`DiscoveryGap`**, **`DiscoveryQuestion`**, **`DiscoveryAgenda`**, **`DiscoveryFinding`**, **`PlanningProposal`** live under `com.vinekeepers.state.planning` (REQ-STATE-001); JSON-serializable for workflow snapshots.

# Phase C (pre-launch critique and approval)

- **Placement:** After Phase B converges (or exits on blocking gaps), **`run_plan_critique_and_readiness`** (`storeSpread`) merges pre-approval visibility keys **`planningThreadReviewBody`** / **`planningThreadReviewBuildError`** (same content as **`build_planning_thread_review_body`**) → branch on **`planReadinessStatus`** (**`BLOCKED`**, **`NEEDS_REVISION`**, **`READY`**, **`NEEDS_HUMAN_DECISION`**) → thread **`post_channel_message`** (orchestrator; includes drafted plan, status, critique) → **`prompt_for_field`** approval choices → **`persist_plan_approval`** → role posts → **`launch_cursor_run`**. Standalone **`build_planning_thread_review_body`** remains registered for legacy YAML.
- **Actions:** **`run_plan_critique_and_readiness`** persists **`PlanCritiqueSnapshot`** and **`PlanConfidence`** on **`FeaturePlanState`**; spreads **`planReadinessStatus`**, **`planReadinessSummary`** (detail counts only; status is not duplicated in notes), **`planCritiqueSummary`**, **`planCritiqueFindingsJson`**, **`planConfidenceLevel`**, plus **`planningThreadReviewBody`** / **`planningThreadReviewBuildError`** from artifact sections (**overall_plan** / **validation_plan** / **project_context**). **`persist_plan_approval`** maps **`planApprovalDecision`** (`approve`, `approve_with_risks`, `revise`, `reject`) to **`PlanApproval`** with **`authorId`** from **`__event`**. **`LaunchCursorRunAction`** when **`FeaturePlanStateStore`** is wired and **`contextId`** is set requires **`PlanApproval`** status **`APPROVE`** or **`APPROVE_WITH_RISKS`** unless **`CURSOR_LAUNCH_SKIP_APPROVAL_GATE=true`**.
