...
| UNIT-CREATE-THREAD-ACTION | CreateThreadActionTest | com.vinekeepers.workflow.actions.CreateThreadActionTest | — | Verify create_thread uses owner gateway when channel has context, setDeliveryTargetId on success when contextId present, no store update on failure, blank channelId returns THREAD_CREATE_FAILED |
...
| UNIT-LAUNCH-CURSOR-RUN-ACTION-MODEL | LaunchCursorRunActionTest | com.vinekeepers.workflow.actions.LaunchCursorRunActionTest | runUsesStepModelWhenConfiguredToSupportedOpenAiModel | Verify launch_cursor_run accepts per-step `model` and forwards supported OpenAI model values |
| UNIT-LAUNCH-CURSOR-RUN-ACTION-MODEL-FALLBACK | LaunchCursorRunActionTest | com.vinekeepers.workflow.actions.LaunchCursorRunActionTest | runFallsBackWhenStepModelIsUnsupported | Verify unsupported per-step model values fall back to `CURSOR_MODEL` |
| UNIT-WORKFLOW-STEP-MODEL-BIND | ConfigurableWorkflowRunnerTest | com.vinekeepers.workflow.ConfigurableWorkflowRunnerTest | callActionStepCopiesTopLevelModelIntoBindArgs | Verify call_action step copies top-level `model` field into action bind args |
| UNIT-CONFIG-LOADER-WORKFLOW-STEP-MODEL | ConfigLoaderTest | com.vinekeepers.config.ConfigLoaderTest | loadFromPathPreservesWorkflowStepModelField | Verify workflow step `model` field is preserved from YAML parsing |
