...
- **`Router`:** collects matching `botId`s from all routing rules, then for Discord channels applies **feature-room** overrides (intake thread → participant bot ids in role order; room channel → coordinator only) before **lifecycle** single-owner precedence; if no override applies, returns deduped filter matches. Depends on **LifecycleContextStore**, optional **FeatureRoomStateStore**, and a per-bot `handlesOwnedSpaces` map (from config).
