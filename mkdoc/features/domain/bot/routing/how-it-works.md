...
`Router` evaluates each incoming `Event` against configured `Routing` rules through `EventFilter` and `RoutingFilter`. `NormalizedEventContext` extracts connector-specific payload details into a shared view (including `actorId` and `actorUsername` from payload author/authorId) so filters can match fields such as actor, channel, text, mentions, repository, and labels consistently.

# Flow

1. A connector publishes an internal `Event`.
2. `NormalizedEventContext` derives normalized routing fields from the payload.
3. `RoutingFilter` evaluates configured criteria such as repositories, labels, channels, `discordTrigger`, `discordMention`, or `discordAuthors` (match by actor id or normalized actor username).
