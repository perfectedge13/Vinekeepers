# Runbooks (nova-wiki)

## Summary

Creates/updates Wiki.js runbooks and sub-pages (operational, troubleshooting, recovery, maintenance); follows page-formats.md.

## Key points

- See **.cursor/skills/nova-wiki/sub-skills/runbooks.md** for full instructions.

## Source (markdown)

<details class="skill-source-wrap">
<summary>Click to expand</summary>

See **.cursor/skills/nova-wiki/sub-skills/runbooks.md** for the full source. Summary: creates/updates Wiki.js runbooks (operational, troubleshooting, recovery, maintenance).

</details>
