...
Captures the expected return shape for the feature-to-domain categorization step. It records the feature mapping plus the per-domain requirement, asset, and feature groupings that feed registry updates.

## Key points

- Shows the `feature_slug -> domain_slug` mapping returned by the categorization step.
- Groups requirement IDs, asset IDs, and feature records per domain for handoff into `update-registry`.
- Serves as an output artifact example rather than the authoritative current project feature roster.
