# Propose registry layout

## Summary

Defines the authoritative registry-file layout for a nova-spec run from the inferred domains plus the configured layout strategy, producing `{ registry_file, domain_slugs[], exists }` entries for the downstream specs-index and registry updates.

## Key points

- Resolves layout strategy from `.cursor/project.yml` (`preserve`, `one_file_per_domain`, or future `explicit`).
- Produces the single authoritative registry layout handoff for steps 2.3 and 2.4.
- Distinguishes existing registry files from new ones that update-registry must create.
