...
Captures the structured handoff produced by the nova-spec scan phase: scan roots, current spec index and registries, candidate assets and requirements, candidate features, and return metadata for the update phase.

## Key points

- Records the scan result as a reusable handoff for update-phase steps 2.1-2.4.
- Includes candidate features with `suggested_domain` plus current spec-state coverage.
- Mirrors the current `.cursor` source so Cursor docs stay aligned with the workflow inputs.
