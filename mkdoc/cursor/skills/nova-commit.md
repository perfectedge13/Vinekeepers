...
Runs the full **verification and commit** workflow: discovery, schema/drift gates, run_tests, plausibility_review, docs_reconcile, build_check, commit_push, output. Do not commit until all prior phases pass, and do not re-run unit tests during `docs_reconcile`.
