...
Checklist applied when syncing the docs dir from specs: create missing docs, update existing docs that lack required sections, markup, or links. Covers index, features, domain pages, feature dossiers, architecture, runbooks, Cursor docs, and mkdocs nav.

## Key points

- **Index and home:** H1 = project name; preserve logo block; Overview; Quick links include Features, Architecture, Runbooks, Cursor.
- **Features:** Definition of a feature; Domains table with link to each domain; each domain page has Features table; each feature has summary + seven sub-pages; diagrams link to main architecture with `../../../../architecture.md`.
- **Architecture:** Overview, System context, Major subsystems, Runtime flows, Diagram; Mermaid uses `flowchart TB`.
- **Runbooks:** All runbook files exist with required headings; linked from index and nav; configuring-bots refreshed from config/bots.yaml when applicable.
- **Cursor docs:** Apply sync-cursor-docs comprehensive checklist (rules, skills, workflows, expandable source, `flowchart TB`).
- **Nav:** Regenerate nav so Features and Cursor sections are complete; remove stale entries.
