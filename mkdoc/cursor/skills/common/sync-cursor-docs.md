...
Syncs Cursor docs from `.cursor/` into the docs dir: rules, skills (including Common), and workflows. Enumerate all .mdc, skill dirs, common .md files, workflow .yml; write cursor/index, rules, skills, common, and workflows pages; update mkdocs.yml nav. Comprehensive gap-fill: create missing pages, update existing ones that lack required sections or expandable source.

## Key points

- **Enumerate:** Rules (.mdc), top-level skills (dirs with SKILL.md), sub-skills, Common (.md except README), nested skills (e.g. nova-code/mk), workflows (.yml).
- **Write:** cursor/index, rules index + per-rule, skills index + per-skill + per-sub-skill, **common index + per-common**, workflows index + per-workflow. Each skill/sub-skill/common page: Summary, Key points, expandable full source (`skill-source-wrap`), Mermaid `flowchart TB`.
- **Nav:** Cursor section with Rules, Skills (each skill + Common with Overview and pages), Workflows. Add/update without removing Features/Architecture/Runbooks when patching.
