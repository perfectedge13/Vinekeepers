# Requirement tracking (common)

## Summary

When to create, update, or split requirements; how to keep ids and traceability consistent. Apply during spec updates (e.g. update-specs sub-skill).

## Key points

- **Create:** New capability/contract, new asset with distinct role, or change out of scope of existing requirement.
- **Update:** Change refines or extends behavior under existing statement/acceptance; update in place.
- **Split:** Requirement too broad, different assets implement different parts, or status/priority differs by sub-behavior. Procedure: new ids, assign tests/traceability, update assets.requires and dependencies, deprecate old requirement.
- **Ids and traceability:** Use registry id convention; update assets.requires, traceability, validation.tests, used_by_requirements; no dangling refs.
