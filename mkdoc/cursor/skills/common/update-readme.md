...
Reviews all 12 artifact categories (implementation, specs, validation, documentation, configuration, commands, data contracts, architecture, operational, dependencies, examples, changelog); updates only impacted. Strong rule: do not assume only code changed—check every category and edit impacted ones. Maintains README and user-facing docs.

## Key points

- **Strong rule:** Do not assume only code changes. Every change triggers review of code, specs, validation, docs, config, operational guidance. All categories checked; only impacted need edits.
- **12 categories:** Implementation, Specs, Validation, Documentation, Configuration, Commands/tooling, Data contracts, Architecture, Operational behavior, Dependencies/integrations, Examples, Change impact records.
- **Instructions:** Determine impacted categories from change summary; update only impacted (README, config, docs, etc.). Summarize what was checked, impacted, and updated.
