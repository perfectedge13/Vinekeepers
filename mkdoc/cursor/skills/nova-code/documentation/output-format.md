...
Defines the required structure for the final `nova-code` run report. It standardizes workflow status reporting, validation results, and the change summary sections that appear in the output step.

## Key points

- Lists every workflow step in execution order with `Pass`, `Skip`, `Fail`, or `Not run`.
- Requires per-step outcomes plus workflow-level validation checkpoints.
- Fixes the final report section order for summary, changed files, specs, gates, tests, static analysis, reconcile, mk, README, and issues.
