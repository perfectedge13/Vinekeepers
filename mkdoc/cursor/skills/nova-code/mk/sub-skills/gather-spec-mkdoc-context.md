# Gather spec and mkdoc context

## Summary

Scans specs (index + registries) and mkdoc feature dossiers for scoped features and compresses the result into one markdown handoff bundle for downstream mk steps (especially mk_feature_dossiers).

## Key points

- Scope: affected_features (or all features when absent).
- Extract from specs per feature: feature id/slug/title/status; requirements (id, title, statement, acceptance criteria, traceability, validation tests); assets (id, path, role).
- Extract from mkdoc: feature summary path and seven sub-pages; excerpt section headings and first 150–250 chars per section.
- Output: single markdown document (compressed context) added to handoff; optional word cap.
...
Produce **one markdown document** so downstream steps can consume it as handoff text. Structure: see sub-skill source for full template (Scope, Per feature with Feature/Requirements/Assets/Existing doc excerpts).
...
- Add to handoff: **compressed_context** = the full markdown string of the document. Alternatively, write to a temp file and pass **compressed_context_path** in handoff.
````
