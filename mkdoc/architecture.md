...
2. A connector publishes an `Event`; `VinekeepersEngine` receives it and asks `Router` to match the event. The Router uses **LifecycleContextStore**, **FeatureRoomStateStore**, and a per-bot `handlesOwnedSpaces` map: when a channel (or delivery target) has **FeatureRoomState**, the Router returns all four participant configuredBotIds (arrietty, architect, auditor, scribe); when a Discord channel has a lifecycle context (no FeatureRoomState) and the owner bot has `handlesOwnedSpaces`, only that bot is returned (single-owner precedence); otherwise the event is matched against normalized routing filters and bot definitions.
