...
| `com.vinekeepers.core` | Engine (workflow first, then reasoner/tool execution), `Bootstrap` (loadConfig → factory creates runners, `registerRunner`; builds handlesMap from `getConnectorIdentity("discord")`; creates `ConnectorRegistry`, `DiscordConnectorAdapter`, calls `adapter.registerBots(bots, ConnectorContext)`; action/sink registration stays in Bootstrap) |
...
| `com.vinekeepers.workflow` | Workflow interface, `WorkflowRunner`, `WorkflowRunnerFactory`, `StubWorkflowRunner`, `ConfigurableWorkflowRunner`, `WorkflowActionRegistry`, `WorkflowDefinition`, `ConfigurableWorkflowState`, `WorkflowTransforms`, `WorkflowConditionEvaluator`, session key helpers, step types (`AskForInputStep`, `PromptForFieldStep`, `CaptureFieldFromEventStep`, `ExtractEventFieldsStep`, `CallActionStep`, `BranchStep`, `DoneStep`), actions **CreateThreadAction** (`create_thread`; returns thread id or `THREAD_CREATE_FAILED`), **PostChannelMessageAction** (`post_channel_message`; send target = deliveryChannelId or channelId), legacy `GatheringState` and `CursorCloudGatheringWorkflow` test harness |
...
| `com.vinekeepers.bot` | Bot definitions, **ConnectorIdentity** (connector-scoped attributes), `RuntimeBotInstance`, **RoutingRule** (filter + botId), conversation/runtime options, normalized routing, persona; `BotDefinition.getConnectorIdentity(connectorId)`; `Router` uses routing rules and `LifecycleContextStore` plus handlesMap from connector identity for ownership-based routing |
| `com.vinekeepers.events` | Event bus, sources, subscribers |
| `com.vinekeepers.connectors` | **ConnectorContext** (EventBus, OutboundDeliveryRouter), **ConnectorAdapter**, **ConnectorRegistry**; generic **ReplySender** and **OutboundGateway** (core uses these); **Discord** implements both (**DiscordReplySender**, **DiscordGateway**); **DiscordConnectorConfig**, **DiscordConnectorAdapter** (per-bot gateway/sender registration); **OutboundDeliveryRouter** (getGatewayForChannel, **getSelfUserIdForBot**); **DiscordAppReplySink** (casts to DiscordGateway for interaction methods); event sources (GitHub, Discord); JDA-backed gateway |
