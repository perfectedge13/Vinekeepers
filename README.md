...
| `com.vinekeepers.bot` | Bot definitions, `RuntimeBotInstance` (provisioned per-channel from template, e.g. Arrietty), **RoutingRule** (filter + botId), conversation/runtime options, normalized routing, persona; `Router` uses routing rules and `LifecycleContextStore` plus per-bot `handlesOwnedSpaces` for ownership-based routing |
