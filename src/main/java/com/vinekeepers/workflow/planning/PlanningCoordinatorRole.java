...
public enum PlanningCoordinatorRole {
    ...
    String systemPromptBlock() {
        String schema = """
                ...
                If you are unsure, return {"upserts":[],"question_if_needed":"","recommended_action":"AUTONOMOUS_REDRAFT"} instead of prose, placeholders, or malformed JSON.
                Schema:
                {
                ...
                If nothing should change, return {"upserts":[],"explicit_assumptions":[],"question_if_needed":"","top_unresolved_gap":"","recommended_action":"AUTONOMOUS_REDRAFT","repo_evidence_this_pass":"not_inspected"}.
                """;
        return switch (this) {
    ...
    String userTaskHint() {
        return switch (this) {
            case COORDINATOR -> "Tighten the draft using repo-grounded facts, explicit assumptions, and the top unresolved gap; ask at most one question only when the canonical next action is ASK_ONE_QUESTION.";
        };
    }
}
